        ...
        <div className="min-h-screen bg-black text-white">
            ...
            <div className="relative z-10">
                ...
                >
                    ...
                    <p className="text-gray-400 text-lg">Fullstack Developer & Software Developer</p>
                    <p className="text-gray-500 text-sm mt-2">Latest writing, featured work, and project links.</p>
                ...
                <motion.div className="max-w-2xl mx-auto px-4 pb-20" variants={containerVariants} initial="hidden" animate="visible">
                    {categories.map((category, categoryIndex) => {
                        const categoryLinks = links.filter((link) => link.category === category);
                        return (
                            <div key={categoryIndex} className="mb-10">
                                <motion.h2 className="text-sm font-semibold text-gray-400 uppercase tracking-widest mb-4 px-4" variants={itemVariants}>
                                    {category}
                                </motion.h2>
                                            ...
                                            <div className="relative px-6 py-4 rounded-lg border border-white/10 bg-white/3 backdrop-blur-sm group-hover:bg-white/8 group-hover:border-blue-400/40 transition-all duration-300">
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-4">
                                                        <div className="text-xl text-blue-400 group-hover:text-blue-300 transition-colors">{link.icon}</div>
                                                        <span className="text-white font-medium group-hover:text-white transition-colors">{link.title}</span>
                                                    </div>
                                                    {link.isExternal && <FaExternalLinkAlt className="text-gray-500 text-xs group-hover:text-blue-400 transition-colors" />}
                                                </div>
                                            </div>
                                        </motion.a>
                                    ))}
